        ...
        <div className='min-h-full w-100% ml-5 flex flex-col items-center justify-center gap-5 overflow-auto'>
            ...
            <div className="flex flex-row shadow-2xl drop-shadow-2xl">
                <div className=" p-10 bg-white border-r">
                    <form class="max-w-5xl" onSubmit={handleSubmit}>
                        <div class="grid md:grid-cols-2 md:gap-6">
                            <div class="relative z-0 w-full mb-5 group">
                                ...
                                onChange={e => setUserInput({ ...userInput, customer_name: e.target.value })}
                                 id="floating_customer_name" className="block py-2.5 px-0 w-full text-sm  bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
                                <label for="floating_customer_name" class="peer-focus:font-medium absolute text-sm  dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Customer Name</label>
                            </div>
                            <div class="relative z-0 w-full mb-5 group">
                                <input 
                                type="text"
                        ...
                        <div className="border-1 my-5 p-3 overflow-y-scroll grid grid-cols-3 h-100 ">
                            ...
                            {data.map((d) => (
                                            <div key={d.id} className="flex flex-col items-center justify-center border-1 h-50 w-40 mb-2"
                                                onClick={() => { setOpen(true); setSelectedProduct(d); }}>
                        ...
                        <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Checkout</button>
                    </form>
                </div>
                {/* Cart Section */}
                                        ...
                                        cartItems.map(item => (
                                            <div key={item.id} className="flex justify-between items-center mb-2">
                                                <div className="flex flex-col">
                                                    <h2 className="text-lg font-semibold text-[#1c1a1a] text-center mb-2">{item.name}</h2>
                                                    <div className="grid grid-cols-3 gap-2 items-center">
                                                        <FaPlus className="cursor-pointer text-[#1c1a1a] hover:text-[#EA1A20] mx-auto"
                                                            onClick={() => handleChangeCartQuantity(item.id, 1)}
                                                        />
                                                        <FaMinus className="cursor-pointer text-[#1c1a1a] hover:text-[#EA1A20] mx-auto"
                                                            onClick={() => handleChangeCartQuantity(item.id, -1)}
                                                        />
                                                ...
                                                <TiDeleteOutline className="text-[#d80c0c] w-5 h-5 cursor-pointer"
