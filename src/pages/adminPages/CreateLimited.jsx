...
function CreateLimited({open, onClose, children}) {
  return (
    <>
    <div onClick={onClose} className={open? " fixed flex items-center justify-center inset-0 transition-colors bg-black/20 " : "hidden"}>
        <div onClick={e => e.stopPropagation()} className={open?"flex flex-col max-w-3xl shadow-2xl bg-white p-10 space-y-3 transition-all scale-100 opacity-100 rounded-2xl" : "scale-125 opacity-0"}> 
