  ...
  <div className='bg-[#EA1A20] top-0 fixed w-[100%]'>  
    <div className='flex justify-between items-center h-20 mx-auto max-w-[1440px] px-4' > 
      ...
      <ul className='text-white hidden uppercase md:flex '>
        ...
        <button className='px-5 py-1 cursor-pointer text-[#000000] bg-[#FFD700] mx-auto rounded-2xl font-semibold'> Order </button>
      </ul>

      <div onClick={navhandler} className=' block md:hidden'> 
        {!nav ? <AiOutlineClose size={20}/> : <AiOutlineMenu size={20}/>}
      </div>
      <div className= {!nav ? 'fixed left-0 top-0 w-[70%] h-full border-r ease-in-out duration-500 bg-[#EA1A20] ' : 'fixed left-[-100%]'}>
      <img  className="pt-3 pl-5 h-16 w-25" src="./src/components/images/logo.png" alt="/" />
